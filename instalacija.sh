...
print_success_message() {
    ...
    echo "  ** Spoljni pristup: podrazumevano sluša na svim interfejsima; otvorite portove u firewall-u (npr. 13000, 13001, 13013, 10000 za fake SMSC)."
    echo "  ** Admin panel:   http://<IP-servera>:13000/"
    echo "  ** HTTP API:      http://<IP-servera>:13013/"
    echo "  ** Log datoteke:  /var/log/kamex/ (ako su podešene)"
    echo "  ** Ako admin panel ne učitava sa mreže: proverite u ${CONFIG_FILE} da nema"
    echo "     admin-deny-ip / admin-allow-ip koji dozvoljavaju samo 127.0.0.1; koristite http:// (ne https) osim ako imate SSL u konfiguraciji."
    echo "  ** Logovi greške: journalctl -u kamex-bearerbox -e --no-pager"
    echo ""
    echo "  Brzi start (sa podrazumevanom konfiguracijom):"
    echo "    bearerbox ${CONFIG_FILE} &"
    echo "    smsbox ${CONFIG_FILE} &"
    echo ""
    echo "  Za testiranje slanja SMS-a (zamenite IP):"
    echo "    curl \"http://<IP-servera>:13013/cgi-bin/sendsms?user=tester&pass=foobar&from=Kamex&to=+1234567890&text=Hello\""
    echo ""
